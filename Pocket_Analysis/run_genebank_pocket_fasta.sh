...
if [[ $# -ne 3 ]]; then
    echo "Usage: $0 <path_to_genbank_script> <directory_to_search> <fasta_directory>"
    echo "Example: $0 ~/Biopython/Pocket_Analysis/dynamic_genbank_pocket.py ./data ./fasta_dir"
    exit 1
fi
...
if [[ ! -d "$FASTA_DIR" ]]; then
    echo "Error: FASTA directory not found at $FASTA_DIR"
    exit 1
fi
...
for summary_file in "$SEARCH_DIR"/*/*_summary.csv; do
    if [[ -f "$summary_file" ]]; then
        ...
        output_file="${dir_name}/${base_name}_pockets.gbk"

        echo "Processing $summary_file..."
        
        # Run the GenBank script
        python "$GENBANK_SCRIPT_PATH" "$summary_file" -f "$FASTA_DIR" -o "$output_file"
