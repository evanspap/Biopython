...
def deduplicate_residues(residues):
    """Extract unique residues by sequence number."""
    unique_residues = {}
    for residue in residues:
        res_seq = residue[3]  # Residue sequence number
        if res_seq not in unique_residues:
            unique_residues[res_seq] = residue
...
def find_close_residues(residues, pocket_atoms, threshold):
    ...
    close_residues = set()  # Use a set to ensure unique residues
    for residue in residues:
        res_coord = residue[4:7]  # (x, y, z)
        for atom_coord in pocket_atoms:
            if calculate_distance(res_coord, atom_coord) < threshold:
                close_residues.add(residue)  # Add to set, which avoids duplicates
                break  # No need to check further for this residue
    return list(close_residues)  # Convert back to a list
...
def main():
    ...
    parser.add_argument(
        "summary_csv",
        help="Full path to the summary CSV file (e.g., './ALG3_reduce_summary.csv')."
    )
    parser.add_argument(
    ...
    unique_residues = deduplicate_residues(residues)

    # Generate sequence
    sequence = "".join([THREE_TO_ONE.get(res[1], "X") for res in unique_residues])  # Single-letter code
    ...
    for index, row in summary_data.iterrows():
        ...
        # Add annotations for the cluster
